...
void Dual_TVL1_optic_flow_multiscale(
		float *I0o,           // source image
		float *I1o,           // target image
		float *u1,           // x component of the optical flow
		float *u2,           // y component of the optical flow
		const int   nxxo,     // image width
		const int   nyyo,     // image height
                const float zfini,    // Initial subsampling
		const float tau,     // time step
		const float lambda,  // weight parameter for the data term
		const float theta,   // weight parameter for (u - v)²
...
)
{
    ...
    //Subsample input imag for SPEED, and blur issues

    int nxx, nyy;
    zoom_size(nxxo,nyyo,&nxx,&nyy,zfini);
   
    //int nxx = ((float)nxxo) * zfini;
    //int nyy = ((float)nyyo) * zfini;

    // allocate memory
    float *I0 = xmalloc(nxx*nyy*sizeof(float));
    float *I1 = xmalloc(nxx*nyy*sizeof(float));

    // zoom in the images to create the pyramidal structure
    zoom_out(I0o, I0, nxxo, nyyo, zfini);
    zoom_out(I1o, I1, nxxo, nyyo, zfini);
	...
	I1s[0] = xmalloc(size*sizeof(float));
        u1s[0] = xmalloc(size*sizeof(float));
        u2s[0] = xmalloc(size*sizeof(float));
	...
	//gaussian(I0s[0], nx[0], ny[0], PRESMOOTHING_SIGMA); //by mdelbra
	//gaussian(I1s[0], nx[0], ny[0], PRESMOOTHING_SIGMA); //by mdelbra
	...
	for (int s = nscales-1; s >= 0; s--) //ACCELERATION mdelbra s>=1
	{
		if (verbose)
    ...
    //Zoom the optical flow to the input scale
	// mauricio's copy, disabled by Shuochen, so that a median filter can be applied on the dwsmp flow images.
    /*
    zoom_in(u1s[0],u1,  nx[0], ny[0], nxxo, nyyo);
    zoom_in(u2s[0],u2,  nx[0], ny[0], nxxo, nyyo);

    for (int i = 0; i < nxxo * nyyo; i++)
    {
        u1[i] *= (float) 1.0 / zfini;
        u2[i] *= (float) 1.0 / zfini;
    }
    */
    // shuochen's modification
    zoom_in(u1s[0],u1,  nx[0], ny[0], nx[0], ny[0]);
    zoom_in(u2s[0],u2,  nx[0], ny[0], nx[0], ny[0]);
    // end of modification by Shuochen
